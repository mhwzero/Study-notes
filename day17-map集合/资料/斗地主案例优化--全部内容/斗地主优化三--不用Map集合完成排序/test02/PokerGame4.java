 ...
 */
public class PokerGame4 {
    public static void main(String[] args) {
        //调用方法准备牌: 获取存储54张扑克的List集合对象
        List<Poker> pokers = zhunBeiPai();
        //调用方法洗牌
        xiPai(pokers);
    ...
    public static void kanPai(List<Player> playerList) {
        for (Player player : playerList) {
            //获取玩家手里的所有的扑克,List集合对象
            List<Poker> pokerList = player.getList();
            //对玩家手里的所有扑克进行排序
            ...
            //获取玩家手里的扑克,不含编号
            List<String> pokerContentList = new ArrayList<>();
            for (Poker poker : pokerList) {
                pokerContentList.add(poker.getContent());
            }
            System.out.println(playerName + ": " + pokerContentList);
        }
    }

    //定义方法发牌
    public static List<Player> faPai(List<Poker> pokers) {
        //三:发牌
    ...
    //定义方法洗牌
    private static void xiPai(List<Poker> pokers) {
    ...
    private static List<Poker> zhunBeiPai() {
        //一:准备牌
        //1.创建List集合对象pokers,存储数据类型Poker,代表牌盒,用来存储54张扑克
        ...
        for (String num : nums) {//增强for遍历数字
            for (String color : colors) {//增强for遍历花色
                //7.拼接一张扑克,存储String变量content中
                String content = color + num;
                //8.把当前拼接的扑克poker添加到牌盒pokers中
                pokers.add(new Poker(index++,content));
            }
        }
        //9.向牌盒pokers中添加小王和大王
        pokers.add(new Poker(index++,"小王"));
        pokers.add(new Poker(index,"大王"));
        //10.打印牌盒
        //System.out.println(pokers);
        //System.out.println(pokers.size());
        return pokers;
